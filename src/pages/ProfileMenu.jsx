...
import { 
  User, Settings, ShieldCheck, LogOut, KeyRound, 
  UserCog, ChevronLeft, ShieldAlert, FileText, ChevronRight,
...
const ProfileMenu = ({ onLogout }) => {
  ...
  const MenuHeader = ({ title }) => (
    <div className={styles["view-header"]}>
      <button className={styles["back-btn"]} onClick={() => setView('menu')}>
        <ChevronLeft size={16} />
      </button>
      <span>{title}</span>
    </div>
  ...
  return (
    <div className={styles["profile-container"]} ref={menuRef}>
      <div className={styles["nav-profile"]} onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className={styles["profile-dropdown"]}>
          ...
          {view === 'menu' && (
            <div className={styles["menu-animate"]}>
              ...
              <div className={styles["dropdown-item"]} onClick={() => setView('details')}>
                <User size={16} /> <span>Account Details</span> <ChevronRight size={14} className={styles["ml-auto"]} />
              </div>
              <div className={styles["dropdown-item"]} onClick={() => setView('settings')}>
              ...
              <div className={styles["dropdown-item"]} onClick={() => setView('password')}>
                <KeyRound size={16} /> <span>Update Password</span>
              </div>
              <div className={styles["dropdown-item"]} onClick={() => setView('policies')}>
                <ShieldAlert size={16} /> <span>Privacy & Policies</span>
              </div>

              <hr className={styles["divider"]} />
              <div className={`${styles["dropdown-item"]} ${styles["logout-item"]}`} onClick={onLogout}>
          ...
          {view === 'details' && (
            <div className={styles["menu-animate"]}>
              <MenuHeader title="Account Details" />
              <div className={styles["inner-content"]}>
                {/* Name Input Handler */}
                <div className={styles["input-group"]}>
                   <label style={{fontSize: '11px', color: '#191b1e'}}>Full Name</label>
                   <input 
          ...
          {view === 'password' && (
            <div className={styles["menu-animate"]}>
              <MenuHeader title="Update Password" />
